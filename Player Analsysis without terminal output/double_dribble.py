        ...
        while self.cap.isOpened():
            ...
            if success:
                ...
                if self.double_dribble_time and time.time() - self.double_dribble_time <= 3:
                    ...
                    # Display a "Double dribble!" message on the frame.
                    cv2.putText(pose_annotated_frame, "Double dribble!", (self.frame_width - 600, 150),
                                cv2.FONT_HERSHEY_SIMPLEX, 2, (255, 255, 255), 4, cv2.LINE_AA)

                # Display the frame.
                cv2.imshow("Basketball Referee AI", pose_annotated_frame)

                # Check if the window has been closed by the user or if 'q' is pressed.
                if cv2.waitKey(1) & 0xFF == ord("q"):
                    break

                # Check if the window was closed manually by the user via the cursor.
