...
from playsound import playsound  # For playing the commentary sound
import pygame
...
class DoubleDribbleDetector:
    def __init__(self):
        ...
        self.cap = cv2.VideoCapture(video_path)
         ...
         # Output video writer
        self.out = None
        self.setup_output_video(video_path)
        ...
        self.commentary_file = "dribble.mp3"  # Path to the commentary file
    ...
    def setup_output_video(self, video_path):
        """Set up the output video file in the same folder as the input video."""
        folder = os.path.dirname(video_path)
        base_name = os.path.splitext(os.path.basename(video_path))[0]
        save_path = os.path.join(folder, f"{base_name}_processed.mp4")
    ...
    def play_commentary(self):
        """Plays the commentary sound in a separate thread."""
        def play_sound():
            try:
                pygame.mixer.init()
                pygame.mixer.music.load(self.commentary_file)
                pygame.mixer.music.play()
            except Exception as e:
                print(f"Error playing commentary sound: {e}")
    ...
    def run(self):
        ...
        while self.cap.isOpened():
            ...
            if success:
                # Process the frame
                pose_annotated_frame, _ = self.process_frame(frame)

                # Check for double dribble
                self.check_double_dribble()

                # Tint frame red if a double dribble was detected
                if self.double_dribble_time and time.time() - self.double_dribble_time <= 3:
                    ...
                    cv2.putText(
                        pose_annotated_frame, "Double dribble!", 
                ...
                if cv2.waitKey(1) & 0xFF == ord("q"):
                    break
                if cv2.getWindowProperty("Basketball Referee AI", cv2.WND_PROP_VISIBLE) < 1:
                    break
            else:
                break
    ...
    def check_double_dribble(self):
        ...
        if self.was_holding and self.dribble_count > 0:
            ...
            self.play_commentary()  # Play the commentary sound

# Create a DoubleDribbleDetector instance and start it.
if __name__ == "__main__":
    app = DoubleDribbleDetector()
    app.run()
